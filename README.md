...
- Contacto incluye formulario conectado a Web3Forms con la integración client-side oficial, manteniendo fallback `mailto:` solo si se desactiva el endpoint.
...
- `forms.contactFormEndpoint`: endpoint de Web3Forms para el formulario de Contacto.
- `forms.collaborationFormEndpoint`: endpoint de Web3Forms para el formulario de Colaborar.

Para enviar con Web3Forms Free, crea un archivo `.env.local` en la raíz del proyecto y define `NEXT_PUBLIC_WEB3FORMS_ACCESS_KEY=MI_CLAVE_REAL`. No pongas la clave real en README, `.env.example` ni archivos públicos versionados.

Si los endpoints se dejan vacíos en `siteConfig.forms`, los formularios mantienen fallback por email. Con Web3Forms activo, el envío se hace por POST directo a `https://api.web3forms.com/submit`.
